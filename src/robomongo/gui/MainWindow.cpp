...
/* -------------------------------- MainWindow --------------------------------- */
    MainWindow::MainWindow()
        ...
        _openAction->setToolTip(
                QString("Load script from the file to the currently opened shell <b>(%1 + O)</b>").arg(controlKey));
        ...
        _saveAction->setToolTip(
                QString("Save script of the currently opened shell to the file <b>(%1 + S)</b>").arg(controlKey));
        ...
        _connectAction->setToolTip(
                QString("Connect to local or remote MongoDB instance <b>(%1 + N)</b>").arg(controlKey));
        ...
        textModeAction->setToolTip(
                "Show current tab in text mode, and make this mode default for all subsequent queries <b>(F4)</b>");
        ...
        treeModeAction->setToolTip(
                "Show current tab in tree mode, and make this mode default for all subsequent queries <b>(F3)</b>");
        ...
        tableModeAction->setToolTip(
                "Show current tab in table mode, and make this mode default for all subsequent queries <b>(F3)</b>");
        ...
        customModeAction->setToolTip(
                "Show current tab in custom mode if possible, and make this mode default for all subsequent queries <b>(F2)</b>");
        ...
        _executeAction->setToolTip(
                QString("Execute query for current tab. If you have some selection in query text - only selection will be executed <b>(F5 </b> or <b>%1 + Enter)</b>").arg(
                        controlKey));
        ...
        csharpLegacyEncodingAction->setChecked(
                AppRegistry::instance().settingsManager()->uuidEncoding() == CSharpLegacy);
        ...
        VERIFY(connect(autocompletionNoCollectionNamesAction, SIGNAL(triggered()), this,
                       SLOT(setShellAutocompletionNoCollectionNames())));
        ...
        VERIFY(connect(connectToolBar->toggleViewAction(), SIGNAL(triggered(bool)), this,
                       SLOT(onConnectToolbarVisibilityChanged(bool))));
        ...
        VERIFY(connect(openSaveToolBar->toggleViewAction(), SIGNAL(triggered(bool)), this,
                       SLOT(onOpenSaveToolbarVisibilityChanged(bool))));
        ...
        VERIFY(connect(_execToolBar->toggleViewAction(), SIGNAL(triggered(bool)), this,
                       SLOT(onExecToolbarVisibilityChanged(bool))));
        ...
        */
        ...
        VERIFY(connect(_networkAccessManager, SIGNAL(finished(QNetworkReply * )), this,
    ...
    void MainWindow::createStatusBar() {
        ...
        QToolButton *history = new QToolButton(this);
        history->setText("History");
        history->setCheckable(true);
        history->setDefaultAction(_historyDock->toggleViewAction());
        ...
        statusBar()->insertWidget(1, history);
    ...
    void MainWindow::manageConnections() {
        ...
        if (result == QDialog::Accepted) {
            ...
            try {
                ...
                QString message = QString("Cannot connect to MongoDB (%1)").arg(
                        QtUtils::toQString(selected->getFullAddress()));
    ...
    */
    ...
    void MainWindow::createDatabaseExplorer() {
        ...
        log->addMessage("Robomonto started...", mongo::logger::LogSeverity::Info());
        VERIFY(connect(&Logger::instance(), SIGNAL(printed(
                       const QString&, mongo::logger::LogSeverity)), log, SLOT(addMessage(const QString&, mongo::logger::LogSeverity))));
        _logDock = new QDockWidget(tr("Logs"));
        _logDock->setAllowedAreas(
...
//        history->addMessage("Aqui escriurem l'historial de queries :P");

        _historyDock = new QDockWidget(tr("History"));
        _historyDock->setAllowedAreas(
                Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea | Qt::BottomDockWidgetArea | Qt::TopDockWidgetArea);
        _historyDock->setWidget(history);
        _historyDock->setFeatures(QDockWidget::DockWidgetClosable);
        _historyDock->setVisible(true);

        QAction *actionHistory = _historyDock->toggleViewAction();
        actionHistory->setText(QString("&History"));
        actionHistory->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_H));
        actionHistory->setChecked(_historyDock->isVisible());
        // Install action in the menu.
        _viewMenu->addAction(actionHistory);

        addDockWidget(Qt::BottomDockWidgetArea, _historyDock);
    ...
    void MainWindow::createTabs() {
        ...
        HistoryWidget *pWidget = dynamic_cast<HistoryWidget *>(_historyDock->widget());
        _workArea->_historyWidget = pWidget;
        ...
        VERIFY(connect(_workArea, SIGNAL(currentChanged(int)), _historyDock, SLOT(on_tabChange())));
